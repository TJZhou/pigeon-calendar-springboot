...
export class DailyScheduleComponent implements OnInit {
  ...
  listEvent(d, e) {
    ...
    for (let i = 0; i < e.length; i++) {
      const eventStartDayOfYear = moment(
        e[i].startTime.substr(0, 10),
        "MM/DD/YYYY"
      ).dayOfYear();
      const eventEndDayOfYear = moment(
        e[i].endTime.substr(0, 10),
        "MM/DD/YYYY"
      ).dayOfYear();
      if (
        curDayOfYear >= eventStartDayOfYear &&
        curDayOfYear <= eventEndDayOfYear
      ) {
  ...
  onDeleteEvent() {
    ...
    this.eventService
      ...
      .subscribe((data) => {
        ...
        for (
          let i = parseInt(event.startTime.substr(11, 2), 0);
          i < parseInt(event.endTime.substr(11, 2), 0);
          i++
        ) {
          this.haveEvent[i] = false;
        }
